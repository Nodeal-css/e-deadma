...
if(isset($_POST['owner'])){
	...
	$sql = "SELECT `plot_id`, `grave_id`, CONCAT(owner.fname, ' ', owner.mi, '. ', owner.lname) AS name, `date_purchased`, `ownership_status`, plot_ownership.owner_id AS plot_owner, owner.owner_id AS owner, `purchase_price`,  `square_meters` FROM `plot_ownership`, `owner` WHERE plot_ownership.owner_id = owner.owner_id AND cemetery_id = '$cemetery' AND CONCAT(owner.fname, ' ', owner.mi, '. ', owner.lname) LIKE '%$owner%';"; 
	...
	if(mysqli_num_rows($result) > 0){
		while($row = mysqli_fetch_assoc($result)){
            if($row['grave_id'] == null){
                ...
                $html .= "<div class='w3-col m3 w3-margin w3-round-xlarge w3-container w3-opacity' style='height: 350px;background-color: rgb(223, 116, 67);color:white;'>
                <table class='w3-table'>
                <tr><td></td><td class='w3-right'><button onclick='openModalView(". $row['plot_id'] .", ". $row['owner'] .");' class='w3-button w3-round w3-white'>View</button></td></tr>
                <tr><td>date: </td><td>" . $row['date_purchased'] . "</td></tr>
                <tr><td>status: </td><td>" . $row['ownership_status'] . "</td></tr>
